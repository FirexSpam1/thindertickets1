...
client.on("ready", () => {
  console.log("Thunder | Logged in! Server count: ${client.guilds.size}");
...
if (message.content.toLowerCase().startsWith(prefix + `new`)) {
    ...
    message.guild.createChannel(`ticket-${message.author.id}`, "text").then(c => {
        ...
        message.channel.send(`:14~2:  **تم فتح تذكرتك | Your Ticket Opened**, #${c.name}.`);
        const embed = new Discord.RichEmbed()
        .setColor(0xCF40FA)
        .addField(`مرحيا ! ${message.author.username}!`, `من فضلك , أشرح لنا سبب فتح هذه التذكرة حتي يأتي فريق الدعم للمساعده.`)
        .setTimestamp();
        c.send({ embed: embed });
    }).catch(console.error);
}
